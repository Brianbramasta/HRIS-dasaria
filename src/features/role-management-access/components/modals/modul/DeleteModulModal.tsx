...
interface DeleteModulModalProps {
  isOpen: boolean;
  onClose: () => void;
  onDelete: () => void;
  modulName?: string;
...
const DeleteModulModal: React.FC<DeleteModulModalProps> = ({
  isOpen,
  onClose,
  onDelete,
  modulName,
        ...
        <form onSubmit={(e) => { e.preventDefault(); onDelete(); }}>
          <div className='max-h-[60vh] overflow-y-auto pb-4'>
            <p className="mb-4 text-center">
              Apakah anda yakin menghapus Modul <strong>{modulName}</strong> ?
            </p>
            <p className='pt-2 text-start text-xs text-error-500'>*Data akan dihapus secara permanen</p>
          </div>
          ...
          <div className="flex justify-end gap-3">
            <button 
              ...
              onClick={onClose} 
              disabled={isLoading}
              className="rounded-xl border px-5 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50"
            >
              Tutup
            </button>
            <button
              type="submit"
              disabled={isLoading}
              className="rounded-xl bg-error-500 hover:bg-error-600 px-5 py-2 text-white disabled:opacity-50 flex items-center gap-2"
            >
...
export default DeleteModulModal;
